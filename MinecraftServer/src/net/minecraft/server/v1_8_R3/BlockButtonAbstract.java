...
public abstract class BlockButtonAbstract extends Block {
    ...
    public boolean interact(World world, BlockPosition blockposition, IBlockData iblockdata, EntityHuman entityhuman, EnumDirection enumdirection, float f, float f1, float f2) {
        if (((Boolean) iblockdata.get(BlockButtonAbstract.POWERED)).booleanValue()) {
            ...
            int i = flag ? 15 : 0;
            int j = !flag ? 15 : 0;
            BlockRedstoneEvent blockredstoneevent = new BlockRedstoneEvent(org_bukkit_block_block, i, j);
    ...
    public void b(World world, BlockPosition blockposition, IBlockData iblockdata, Random random) {
        if (!world.isClientSide && ((Boolean) iblockdata.get(BlockButtonAbstract.POWERED)).booleanValue()) {
            if (this.N) {
                ...
                org.bukkit.block.Block org_bukkit_block_block = world.getWorld().getBlockAt(blockposition.getX(), blockposition.getY(), blockposition.getZ());
                BlockRedstoneEvent blockredstoneevent = new BlockRedstoneEvent(org_bukkit_block_block, 15, 0);
    ...
    public void a(World world, BlockPosition blockposition, IBlockData iblockdata, Entity entity) {
        if (!world.isClientSide && this.N && !((Boolean) iblockdata.get(BlockButtonAbstract.POWERED)).booleanValue()) {
            this.f(world, blockposition, iblockdata);
        }
    ...
    private void f(World world, BlockPosition blockposition, IBlockData iblockdata) {
        ...
        if (flag1 != flag && flag) {
            org_bukkit_block_block = world.getWorld().getBlockAt(blockposition.getX(), blockposition.getY(), blockposition.getZ());
            boolean flag2 = false;
            ...
            while (iterator.hasNext()) {
                ...
                if (object != null) {
                    EntityInteractEvent entityinteractevent = new EntityInteractEvent(((Entity) object).getBukkitEntity(), org_bukkit_block_block);
        ...
        if (flag && !flag1) {
            org_bukkit_block_block = world.getWorld().getBlockAt(blockposition.getX(), blockposition.getY(), blockposition.getZ());
            blockredstoneevent = new BlockRedstoneEvent(org_bukkit_block_block, 0, 15);
            world.getServer().getPluginManager().callEvent(blockredstoneevent);
            if (blockredstoneevent.getNewCurrent() <= 0) {
                return;
            }
        ...
        if (!flag && flag1) {
            org_bukkit_block_block = world.getWorld().getBlockAt(blockposition.getX(), blockposition.getY(), blockposition.getZ());
            blockredstoneevent = new BlockRedstoneEvent(org_bukkit_block_block, 15, 0);
            world.getServer().getPluginManager().callEvent(blockredstoneevent);
            if (blockredstoneevent.getNewCurrent() > 0) {
                return;
            }
