...
public class BlockCake extends Block {
    ...
    private void b(World world, BlockPosition blockposition, IBlockData iblockdata, EntityHuman entityhuman) {
        if (entityhuman.j(false)) {
            ...
            FoodLevelChangeEvent foodlevelchangeevent = CraftEventFactory.callFoodLevelChangeEvent(entityhuman, 2 + i);

            if (!foodlevelchangeevent.isCancelled()) {
                entityhuman.getFoodData().eat(foodlevelchangeevent.getFoodLevel() - i, 0.1F);
            }

            ((EntityPlayer) entityhuman).playerConnection.sendPacket(new PacketPlayOutUpdateHealth(((EntityPlayer) entityhuman).getBukkitEntity().getScaledHealth(), entityhuman.getFoodData().foodLevel, entityhuman.getFoodData().saturationLevel));
            int j = ((Integer) iblockdata.get(BlockCake.BITES)).intValue();

            if (j < 6) {
                world.setTypeAndData(blockposition, iblockdata.set(BlockCake.BITES, Integer.valueOf(j + 1)), 3);
            } else {
