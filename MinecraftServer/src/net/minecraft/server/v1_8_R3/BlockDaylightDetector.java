    ...
    public void f(World world, BlockPosition blockposition) {
        if (!world.worldProvider.o()) {
            ...
            if (((Integer) iblockdata.get(BlockDaylightDetector.POWER)).intValue() != i) {
                i = CraftEventFactory.callRedstoneChange(world, blockposition.getX(), blockposition.getY(), blockposition.getZ(), ((Integer) iblockdata.get(BlockDaylightDetector.POWER)).intValue(), i).getNewCurrent();
    ...
    public boolean interact(World world, BlockPosition blockposition, IBlockData iblockdata, EntityHuman entityhuman, EnumDirection enumdirection, float f, float f1, float f2) {
        if (entityhuman.cn()) {
            if (world.isClientSide) {
                ...
                if (this.b) {
                    world.setTypeAndData(blockposition, Blocks.DAYLIGHT_DETECTOR.getBlockData().set(BlockDaylightDetector.POWER, (Integer) iblockdata.get(BlockDaylightDetector.POWER)), 4);
                    Blocks.DAYLIGHT_DETECTOR.f(world, blockposition);
                } else {
                    world.setTypeAndData(blockposition, Blocks.DAYLIGHT_DETECTOR_INVERTED.getBlockData().set(BlockDaylightDetector.POWER, (Integer) iblockdata.get(BlockDaylightDetector.POWER)), 4);
