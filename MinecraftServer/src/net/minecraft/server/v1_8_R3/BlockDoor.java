...
public class BlockDoor extends Block {
    ...
    public void doPhysics(World world, BlockPosition blockposition, IBlockData iblockdata, Block block) {
        if (iblockdata.get(BlockDoor.HALF) == BlockDoor.EnumDoorHalf.UPPER) {
            ...
            if (flag) {
                ...
                org.bukkit.block.Block org_bukkit_block_block = craftworld.getBlockAt(blockposition.getX(), blockposition.getY(), blockposition.getZ());
                org.bukkit.block.Block org_bukkit_block_block1 = craftworld.getBlockAt(blockposition2.getX(), blockposition2.getY(), blockposition2.getZ());
                int i = org_bukkit_block_block.getBlockPower();
                int j = org_bukkit_block_block1.getBlockPower();

                if (j > i) {
                    i = j;
                }

                int k = ((Boolean) iblockdata2.get(BlockDoor.POWERED)).booleanValue() ? 15 : 0;

                if (k == 0 ^ i == 0) {
                    BlockRedstoneEvent blockredstoneevent = new BlockRedstoneEvent(org_bukkit_block_block, k, i);

                    world.getServer().getPluginManager().callEvent(blockredstoneevent);
                    boolean flag1 = blockredstoneevent.getNewCurrent() > 0;
    ...
    public IBlockData updateState(IBlockData iblockdata, IBlockAccess iblockaccess, BlockPosition blockposition) {
        ...
        if (iblockdata.get(BlockDoor.HALF) == BlockDoor.EnumDoorHalf.LOWER) {
            ...
            if (iblockdata1.getBlock() == this) {
                iblockdata = iblockdata.set(BlockDoor.HINGE, (BlockDoor.EnumDoorHinge) iblockdata1.get(BlockDoor.HINGE)).set(BlockDoor.POWERED, (Boolean) iblockdata1.get(BlockDoor.POWERED));
            }
        } else {
            iblockdata1 = iblockaccess.getType(blockposition.down());
            if (iblockdata1.getBlock() == this) {
                iblockdata = iblockdata.set(BlockDoor.FACING, (EnumDirection) iblockdata1.get(BlockDoor.FACING)).set(BlockDoor.OPEN, (Boolean) iblockdata1.get(BlockDoor.OPEN));
            }
        }
