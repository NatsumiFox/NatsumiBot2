...
public class BlockStem extends BlockPlant implements IBlockFragilePlantElement {
    ...
    public void b(World world, BlockPosition blockposition, IBlockData iblockdata, Random random) {
        ...
        if (world.getLightLevel(blockposition.up()) >= 9) {
            ...
            if (random.nextInt((int) (world.growthOdds / (float) (this == Blocks.PUMPKIN_STEM ? world.spigotConfig.pumpkinModifier : world.spigotConfig.melonModifier) * (25.0F / f)) + 1) == 0) {
                int i = ((Integer) iblockdata.get(BlockStem.AGE)).intValue();

                if (i < 7) {
                    iblockdata = iblockdata.set(BlockStem.AGE, Integer.valueOf(i + 1));
                    CraftEventFactory.handleBlockGrowEvent(world, blockposition.getX(), blockposition.getY(), blockposition.getZ(), this, this.toLegacyData(iblockdata));
                } else {
                    Iterator iterator = EnumDirection.EnumDirectionLimit.HORIZONTAL.iterator();
