        ...
        if (!world.isClientSide) {
            ...
            if (!c(world.getType(blockposition1).getBlock())) {
                ...
                if (flag || block.isPowerSource()) {
                    ...
                    int j = ((Boolean) iblockdata.get(BlockTrapdoor.OPEN)).booleanValue() ? 15 : 0;

                    if (j == 0 ^ i == 0 || block.isPowerSource()) {
                        BlockRedstoneEvent blockredstoneevent = new BlockRedstoneEvent(org_bukkit_block_block, j, i);
