...
public class ContainerEnchantTable extends Container {
    ...
    public boolean a(EntityHuman entityhuman, int i) {
        ...
        if ((itemstack1 == null || itemstack1.count < j) && !entityhuman.abilities.canInstantlyBuild) {
            ...
            if (!this.world.isClientSide) {
                Object object = this.a(itemstack, i, this.costs[i]);
                ...
                if (object != null) {
                    ...
                    while (iterator.hasNext()) {
                        ...
                        WeightedRandomEnchant weightedrandomenchant = (WeightedRandomEnchant) object1;

                        hashmap.put(org.bukkit.enchantments.Enchantment.getById(weightedrandomenchant.enchantment.id), Integer.valueOf(weightedrandomenchant.level));
                    }

                    CraftItemStack craftitemstack = CraftItemStack.asCraftMirror(itemstack);
                    EnchantItemEvent enchantitemevent = new EnchantItemEvent((Player) entityhuman.getBukkitEntity(), this.getBukkitView(), this.world.getWorld().getBlockAt(this.position.getX(), this.position.getY(), this.position.getZ()), craftitemstack, this.costs[i], hashmap, i);

                    this.world.getServer().getPluginManager().callEvent(enchantitemevent);
                    int k = enchantitemevent.getExpLevelCost();

                    if (enchantitemevent.isCancelled() || k > entityhuman.expLevel && !entityhuman.abilities.canInstantlyBuild || enchantitemevent.getEnchantsToAdd().isEmpty()) {
                        return false;
                    }
                    ...
                    Iterator iterator1 = enchantitemevent.getEnchantsToAdd().entrySet().iterator();
                    ...
                    while (iterator1.hasNext()) {
                        ...
                        try {
                            if (flag) {
                                int l = ((org.bukkit.enchantments.Enchantment) entry.getKey()).getId();

                                if (Enchantment.getById(l) != null) {
                                    WeightedRandomEnchant weightedrandomenchant1 = new WeightedRandomEnchant(Enchantment.getById(l), ((Integer) entry.getValue()).intValue());
    ...
    public boolean a(EntityHuman entityhuman) {
        return !this.checkReachable ? true : (this.world.getType(this.position).getBlock() != Blocks.ENCHANTING_TABLE ? false : entityhuman.e((double) this.position.getX() + 0.5D, (double) this.position.getY() + 0.5D, (double) this.position.getZ() + 0.5D) <= 64.0D);
    }
