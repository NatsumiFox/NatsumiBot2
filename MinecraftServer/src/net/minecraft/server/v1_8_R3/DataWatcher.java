...
public class DataWatcher {
    ...
    private static final Map<Class<?>, Integer> c = TDecorators.wrap(DataWatcher.classToId);
    private final Map<Integer, DataWatcher.WatchableObject> d;
    private boolean e;
    private ReadWriteLock f;

    static {
        DataWatcher.classToId.put(Byte.class, 0);
        DataWatcher.classToId.put(Short.class, 1);
        DataWatcher.classToId.put(Integer.class, 2);
        DataWatcher.classToId.put(Float.class, 3);
        DataWatcher.classToId.put(String.class, 4);
        DataWatcher.classToId.put(ItemStack.class, 5);
        DataWatcher.classToId.put(BlockPosition.class, 6);
        DataWatcher.classToId.put(Vector3f.class, 7);
    }
    ...
    public <T> void a(int i, T t0) {
        int j = DataWatcher.classToId.get(t0.getClass());
        ...
        if (j == -1) {
            ...
            DataWatcher.WatchableObject datawatcher_watchableobject = new DataWatcher.WatchableObject(j, i, t0);
    ...
    private DataWatcher.WatchableObject j(int i) {
        ...
        try {
            datawatcher_watchableobject = (DataWatcher.WatchableObject) this.dataValues.get(i);
            ...
            while (iterator.hasNext()) {
                ...
                if (datawatcher_watchableobject.d()) {
                    ...
                    if (datawatcher_watchableobject.b() instanceof ItemStack) {
                        datawatcher_watchableobject = new DataWatcher.WatchableObject(datawatcher_watchableobject.c(), datawatcher_watchableobject.a(), ((ItemStack) datawatcher_watchableobject.b()).cloneItemStack());
                    }
    ...
    public List<DataWatcher.WatchableObject> c() {
        ...
        for (int i = 0; i < arraylist.size(); ++i) {
            DataWatcher.WatchableObject datawatcher_watchableobject = (DataWatcher.WatchableObject) arraylist.get(i);

            if (datawatcher_watchableobject.b() instanceof ItemStack) {
                datawatcher_watchableobject = new DataWatcher.WatchableObject(datawatcher_watchableobject.c(), datawatcher_watchableobject.a(), ((ItemStack) datawatcher_watchableobject.b()).cloneItemStack());
                arraylist.set(i, datawatcher_watchableobject);
            }
        }
