...
public abstract class Entity implements ICommandListener {
    ...
    static boolean isLevelAtLeast(NBTTagCompound nbttagcompound, int i) {
        ...
        if (!this.fireProof) {
            ...
            if (this instanceof EntityLiving) {
                if (this.fireTicks <= 0) {
                    ...
                    EntityCombustByBlockEvent entitycombustbyblockevent = new EntityCombustByBlockEvent((org.bukkit.block.Block) object, craftentity, 15);

                    this.world.getServer().getPluginManager().callEvent(entitycombustbyblockevent);
                    if (!entitycombustbyblockevent.isCancelled()) {
                        this.setOnFire(entitycombustbyblockevent.getDuration());
                    }
                } else {
                    this.setOnFire(15);
                }
    ...
    public void move(double d0, double d1, double d2) {
        ...
        if (this.noclip) {
            ...
            if (this.positionChanged && this.getBukkitEntity() instanceof Vehicle) {
                ...
                org.bukkit.block.Block org_bukkit_block_block = this.world.getWorld().getBlockAt(MathHelper.floor(this.locX), MathHelper.floor(this.locY), MathHelper.floor(this.locZ));

                if (d6 > d0) {
                    org_bukkit_block_block = org_bukkit_block_block.getRelative(BlockFace.EAST);
                } else if (d6 < d0) {
                    org_bukkit_block_block = org_bukkit_block_block.getRelative(BlockFace.WEST);
                } else if (d8 > d2) {
                    org_bukkit_block_block = org_bukkit_block_block.getRelative(BlockFace.SOUTH);
                } else if (d8 < d2) {
                    org_bukkit_block_block = org_bukkit_block_block.getRelative(BlockFace.NORTH);
                }

                VehicleBlockCollisionEvent vehicleblockcollisionevent = new VehicleBlockCollisionEvent(vehicle, org_bukkit_block_block);
    ...
    protected void burn(float f) {
        if (!this.fireProof) {
            this.damageEntity(DamageSource.FIRE, f);
        }
    ...
    public void f(NBTTagCompound nbttagcompound) {
        try {
            ...
            if (this instanceof EntityPlayer) {
                ...
                String s = nbttagcompound.getString("world");
                Object object;

                if (nbttagcompound.hasKey("WorldUUIDMost") && nbttagcompound.hasKey("WorldUUIDLeast")) {
                    UUID uuid = new UUID(nbttagcompound.getLong("WorldUUIDMost"), nbttagcompound.getLong("WorldUUIDLeast"));

                    object = server.getWorld(uuid);
                } else {
                    object = server.getWorld(s);
                }

                if (object == null) {
                    EntityPlayer entityplayer = (EntityPlayer) this;

                    object = ((CraftServer) server).getServer().getWorldServer(entityplayer.dimension).getWorld();
                }

                this.spawnIn(object == null ? null : ((CraftWorld) object).getHandle());
            }
    ...
    public void mount(Entity entity) {
        ...
        Entity entity2 = this.vehicle == null ? null : this.vehicle.passenger;
        ...
        if (entity == null) {
            if (this.vehicle != null) {
                if (this.bukkitEntity instanceof LivingEntity && this.vehicle.getBukkitEntity() instanceof Vehicle) {
                    vehicleexitevent = new VehicleExitEvent((Vehicle) this.vehicle.getBukkitEntity(), (LivingEntity) this.bukkitEntity);
            ...
            if (this.bukkitEntity instanceof LivingEntity && entity.getBukkitEntity() instanceof Vehicle && entity.world.isChunkLoaded((int) entity.locX >> 4, (int) entity.locZ >> 4, true)) {
                vehicleexitevent = null;
                if (this.vehicle != null && this.vehicle.getBukkitEntity() instanceof Vehicle) {
                    vehicleexitevent = new VehicleExitEvent((Vehicle) this.vehicle.getBukkitEntity(), (LivingEntity) this.bukkitEntity);
                    pluginmanager.callEvent(vehicleexitevent);
                    if (vehicleexitevent.isCancelled() || this.vehicle != entity1 || this.vehicle != null && this.vehicle.passenger != entity2) {
                        return;
                    }
                }
                ...
                if (vehicleenterevent.isCancelled() || this.vehicle != entity1 || this.vehicle != null && this.vehicle.passenger != entity2) {
                    if (vehicleexitevent != null && this.vehicle == entity1 && this.vehicle != null && this.vehicle.passenger == entity2) {
                        this.setPositionRotation(this.vehicle.locX, this.vehicle.getBoundingBox().b + (double) this.vehicle.length, this.vehicle.locZ, this.yaw, this.pitch);
                        this.vehicle.passenger = null;
                        this.vehicle = null;
                    }
            ...
            if (entity.world.isChunkLoaded((int) entity.locX >> 4, (int) entity.locZ >> 4, true)) {
                EntityMountEvent entitymountevent = new EntityMountEvent(this.getBukkitEntity(), entity.getBukkitEntity());
            ...
            if (entity != null) {
                for (Entity entity3 = entity.vehicle; entity3 != null; entity3 = entity3.vehicle) {
                    if (entity3 == this) {
                        return;
                    }
                }
    ...
    public void onLightningStrike(EntityLightning entitylightning) {
        CraftEntity craftentity = this.getBukkitEntity();
        CraftEntity craftentity1 = entitylightning.getBukkitEntity();
        PluginManager pluginmanager = Bukkit.getPluginManager();

        if (craftentity instanceof Hanging) {
            HangingBreakByEntityEvent hangingbreakbyentityevent = new HangingBreakByEntityEvent((Hanging) craftentity, craftentity1);
            PaintingBreakByEntityEvent paintingbreakbyentityevent = null;

            if (craftentity instanceof Painting) {
                paintingbreakbyentityevent = new PaintingBreakByEntityEvent((Painting) craftentity, craftentity1);
            }
        ...
        if (!this.fireProof) {
            ...
            if (!this.damageEntity(DamageSource.LIGHTNING, 5.0F)) {
                ...
                ++this.fireTicks;
                if (this.fireTicks == 0) {
                    EntityCombustByEntityEvent entitycombustbyentityevent = new EntityCombustByEntityEvent(craftentity1, craftentity, 8);
    ...
    public void c(int i) {
        if (!this.world.isClientSide && !this.dead) {
            ...
            if (this.dimension < 10) {
                Iterator iterator = minecraftserver.worlds.iterator();
            ...
            Location location1 = worldserver != null ? minecraftserver.getPlayerList().calculateTarget(location, minecraftserver.getWorldServer(i)) : null;
            ...
            EntityPortalEvent entityportalevent = new EntityPortalEvent(this.getBukkitEntity(), location, location1, travelagent);
            ...
            location1 = entityportalevent.useTravelAgent() ? entityportalevent.getPortalTravelAgent().findOrCreate(entityportalevent.getTo()) : entityportalevent.getTo();
            this.teleportTo(location1, true);
        }

    }

    public void teleportTo(Location location, boolean flag) {
        ...
        boolean flag1 = worldserver1.chunkProviderServer.forceChunkLoad;

        worldserver1.chunkProviderServer.forceChunkLoad = true;
        worldserver1.getMinecraftServer().getPlayerList().repositionEntity(this, location, flag);
        worldserver1.chunkProviderServer.forceChunkLoad = flag1;
    ...
    public void a(AxisAlignedBB axisalignedbb) {
        ...
        double d6 = axisalignedbb.d - axisalignedbb.a;
        ...
        d6 = axisalignedbb.e - axisalignedbb.b;
        if (d6 < 0.0D) {
            d4 = d1;
        }
        ...
        d6 = axisalignedbb.f - axisalignedbb.c;
        if (d6 < 0.0D) {
            d5 = d2;
        }
