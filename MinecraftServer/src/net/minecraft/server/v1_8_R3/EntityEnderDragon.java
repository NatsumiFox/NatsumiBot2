...
public class EntityEnderDragon extends EntityInsentient implements IComplex, IMonster {
            ...
            if (this.ce()) {
                ...
                if (this.world.isClientSide) {
                    ...
                    if (this.bA != null) {
                        ...
                        d6 = 0.4000000059604645D + d7 / 80.0D - 1.0D;
                        if (d6 > 10.0D) {
                            d6 = 10.0D;
                        }

                        this.b = this.bA.getBoundingBox().b + d6;
                    } else {
                    ...
                    this.yaw = MathHelper.g(this.yaw);
                    d4 = 180.0D - MathHelper.b(d0, d2) * 180.0D / 3.1415927410125732D;
                    d5 = MathHelper.g(d4 - (double) this.yaw);
                    if (d5 > 50.0D) {
                        d5 = 50.0D;
                    }
                    ...
                    d6 = (double) (-MathHelper.cos(this.yaw * 3.1415927F / 180.0F));
                    Vec3D vec3d1 = (new Vec3D((double) MathHelper.sin(this.yaw * 3.1415927F / 180.0F), this.motY, d6)).a();
                    float f6 = ((float) vec3d1.b(vec3d) + 0.5F) / 1.5F;
                    ...
                    this.bb *= 0.8F;
                    float f7 = MathHelper.sqrt(this.motX * this.motX + this.motZ * this.motZ) * 1.0F + 1.0F;
                    ...
                    this.bb = (float) ((double) this.bb + d5 * (0.699999988079071D / d8 / (double) f7));
                    this.yaw += this.bb * 0.1F;
                    f4 = (float) (2.0D / (d8 + 1.0D));
                    f5 = 0.06F;
                    this.a(0.0F, -1.0F, f5 * (f6 * f4 + (1.0F - f4)));
                    if (this.bx) {
                ...
                for (int j = 0; j < 3; ++j) {
                    ...
                    f4 = MathHelper.cos(f14);
                    f5 = 1.5F;
                    float f16 = (float) (j + 1) * 2.0F;

                    entitycomplexpart.t_();
                    entitycomplexpart.setPositionRotation(this.locX - (double) ((f11 * f5 + f15 * f16) * f2), this.locY + (adouble2[1] - adouble[1]) * 1.0D - (double) ((f16 + f5) * f9) + 1.5D, this.locZ + (double) ((f12 * f5 + f4 * f16) * f2), 0.0F, 0.0F);
                }
        ...
        if (this.random.nextInt(2) == 0 && !arraylist.isEmpty()) {
            ...
            if (!entitytargetevent.isCancelled()) {
                if (entitytargetevent.getTarget() == null) {
                    ...
                    this.bA = ((CraftEntity) entitytargetevent.getTarget()).getHandle();
                }
            }
        } else {
            boolean flag;
    ...
    private boolean b(AxisAlignedBB axisalignedbb) {
                ...
                for (int i2 = k; i2 <= j1; ++i2) {
                    ...
                    if (block.getMaterial() != Material.AIR) {
                        if (block != Blocks.BARRIER && block != Blocks.OBSIDIAN && block != Blocks.END_STONE && block != Blocks.BEDROCK && block != Blocks.COMMAND_BLOCK && this.world.getGameRules().getBoolean("mobGriefing")) {
                            flag1 = true;
                            arraylist.add(craftworld.getBlockAt(k1, l1, i2));
                        } else {
                            flag = true;
                        }
                    }
        ...
        if (flag1) {
            ...
            EntityExplodeEvent entityexplodeevent = new EntityExplodeEvent(craftentity, craftentity.getLocation(), arraylist, 0.0F);
            ...
            org.bukkit.block.Block org_bukkit_block_block;
            Iterator iterator;

            if (entityexplodeevent.getYield() == 0.0F) {
                iterator = entityexplodeevent.blockList().iterator();

                while (iterator.hasNext()) {
                    org_bukkit_block_block = (org.bukkit.block.Block) iterator.next();
                    this.world.setAir(new BlockPosition(org_bukkit_block_block.getX(), org_bukkit_block_block.getY(), org_bukkit_block_block.getZ()));
                }
            } else {
                iterator = entityexplodeevent.blockList().iterator();
                ...
                while (iterator.hasNext()) {
                    org_bukkit_block_block = (org.bukkit.block.Block) iterator.next();
                    org.bukkit.Material org_bukkit_material = org_bukkit_block_block.getType();

                    if (org_bukkit_material != org.bukkit.Material.AIR) {
                        int j2 = org_bukkit_block_block.getX();
                        int k2 = org_bukkit_block_block.getY();
                        int l2 = org_bukkit_block_block.getZ();
                        Block block1 = CraftMagicNumbers.getBlock(org_bukkit_material);

                        if (block1.a(this.explosionSource)) {
                            block1.dropNaturally(this.world, new BlockPosition(j2, k2, l2), block1.fromLegacyData(org_bukkit_block_block.getData()), entityexplodeevent.getYield(), 0);
                        }
    ...
    protected void aZ() {
        if (!this.dead) {
            ...
            if (!this.world.isClientSide) {
                ...
                if (this.by == 1) {
                    int k = ((WorldServer) this.world).getServer().getViewDistance() * 16;
                    ...
                    while (iterator.hasNext()) {
                        ...
                        if (this.world.spigotConfig.dragonDeathSoundRadius <= 0 || d2 <= (double) (this.world.spigotConfig.dragonDeathSoundRadius * this.world.spigotConfig.dragonDeathSoundRadius)) {
                            if (d2 > (double) (k * k)) {
                                ...
                                double d4 = entityplayer.locX + d0 / d3 * (double) k;
                                double d5 = entityplayer.locZ + d1 / d3 * (double) k;
    ...
    private void a(BlockPosition blockposition) {
                ...
                for (int k = -4; k <= 4; ++k) {
                    ...
                    if (d0 <= 12.25D) {
                        ...
                        if (i < 0) {
                            ...
                            blockstatelistpopulator.setTypeUpdate(blockposition1, Blocks.AIR.getBlockData());
                        } else if (d0 > 6.25D) {
                            blockstatelistpopulator.setTypeUpdate(blockposition1, Blocks.BEDROCK.getBlockData());
                        } else {
                            blockstatelistpopulator.setTypeUpdate(blockposition1, Blocks.END_PORTAL.getBlockData());
                        }
                    }
                }
            }
        ...
        blockstatelistpopulator.setTypeUpdate(blockposition2, Blocks.BEDROCK.getBlockData());
        blockstatelistpopulator.setTypeUpdate(blockposition2.west(), Blocks.TORCH.getBlockData().set(BlockTorch.FACING, EnumDirection.EAST));
        blockstatelistpopulator.setTypeUpdate(blockposition2.east(), Blocks.TORCH.getBlockData().set(BlockTorch.FACING, EnumDirection.WEST));
        blockstatelistpopulator.setTypeUpdate(blockposition2.north(), Blocks.TORCH.getBlockData().set(BlockTorch.FACING, EnumDirection.SOUTH));
        blockstatelistpopulator.setTypeUpdate(blockposition2.south(), Blocks.TORCH.getBlockData().set(BlockTorch.FACING, EnumDirection.NORTH));
        ...
        if (!entitycreateportalevent.isCancelled()) {
            iterator = entitycreateportalevent.getBlocks().iterator();

            while (iterator.hasNext()) {
                org_bukkit_block_blockstate = (org.bukkit.block.BlockState) iterator.next();
            ...
            while (iterator.hasNext()) {
                org_bukkit_block_blockstate = (org.bukkit.block.BlockState) iterator.next();
                PacketPlayOutBlockChange packetplayoutblockchange = new PacketPlayOutBlockChange(this.world, new BlockPosition(org_bukkit_block_blockstate.getX(), org_bukkit_block_blockstate.getY(), org_bukkit_block_blockstate.getZ()));
                Iterator iterator1 = this.world.players.iterator();
