...
public abstract class EntityInsentient extends EntityLiving {
    ...
    public void setGoalTarget(EntityLiving entityliving, TargetReason targetreason, boolean flag) {
        if (this.getGoalTarget() != entityliving) {
            if (flag) {
                if (targetreason == TargetReason.UNKNOWN && this.getGoalTarget() != null && entityliving == null) {
                    targetreason = this.getGoalTarget().isAlive() ? TargetReason.FORGOT_TARGET : TargetReason.TARGET_DIED;
                }

                if (targetreason == TargetReason.UNKNOWN) {
                ...
                EntityTargetLivingEntityEvent entitytargetlivingentityevent = new EntityTargetLivingEntityEvent(this.getBukkitEntity(), craftlivingentity, targetreason);
    ...
    public void a(NBTTagCompound nbttagcompound) {
        ...
        if (nbttagcompound.hasKeyOfType("CanPickUpLoot", 1)) {
            flag = nbttagcompound.getBoolean("CanPickUpLoot");
            if (isLevelAtLeast(nbttagcompound, 1) || flag) {
                this.j(flag);
            }
        }

        flag = nbttagcompound.getBoolean("PersistenceRequired");
        if (isLevelAtLeast(nbttagcompound, 1) || flag) {
