        ...
        if (!this.world.isClientSide) {
            ...
            for (int l = 0; l < 4; ++l) {
                ...
                if (this.world.getType(blockposition).getBlock().getMaterial() == Material.AIR && this.world.getBiome(new BlockPosition(i, 0, k)).a(blockposition) < 0.8F && Blocks.SNOW_LAYER.canPlace(this.world, blockposition)) {
                    org.bukkit.block.BlockState org_bukkit_block_blockstate = this.world.getWorld().getBlockAt(i, j, k).getState();

                    org_bukkit_block_blockstate.setType(CraftMagicNumbers.getMaterial(Blocks.SNOW_LAYER));
                    EntityBlockFormEvent entityblockformevent = new EntityBlockFormEvent(this.getBukkitEntity(), org_bukkit_block_blockstate.getBlock(), org_bukkit_block_blockstate);
