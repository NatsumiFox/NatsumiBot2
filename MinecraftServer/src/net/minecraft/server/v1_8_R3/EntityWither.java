    ...
    public void m() {
        ...
        for (i = 0; i < 2; ++i) {
            ...
            if (entity1 != null) {
                ...
                d2 = this.v(i + 1);
                d4 = entity1.locX - d0;
                d5 = entity1.locY + (double) entity1.getHeadHeight() - d1;
                d6 = entity1.locZ - d2;
        ...
        for (j = 0; j < 3; ++j) {
            ...
            d5 = this.u(j);
            d6 = this.v(j);
            this.world.addParticle(EnumParticle.SMOKE_NORMAL, d4 + this.random.nextGaussian() * 0.30000001192092896D, d5 + this.random.nextGaussian() * 0.30000001192092896D, d6 + this.random.nextGaussian() * 0.30000001192092896D, 0.0D, 0.0D, 0.0D, new int[0]);
            if (flag && this.world.random.nextInt(4) == 0) {
                this.world.addParticle(EnumParticle.SPELL_MOB, d4 + this.random.nextGaussian() * 0.30000001192092896D, d5 + this.random.nextGaussian() * 0.30000001192092896D, d6 + this.random.nextGaussian() * 0.30000001192092896D, 0.699999988079071D, 0.699999988079071D, 0.5D, new int[0]);
            }
        }
    ...
    protected void E() {
        ...
        if (this.cl() > 0) {
            ...
            if (i <= 0) {
                ...
                j = ((WorldServer) this.world).getServer().getViewDistance() * 16;
            ...
            for (i = 1; i < 3; ++i) {
                if (this.ticksLived >= this.bn[i - 1]) {
                    ...
                    if (this.world.getDifficulty() == EnumDifficulty.NORMAL || this.world.getDifficulty() == EnumDifficulty.HARD) {
                        j = i - 1;
                        l = this.bo[i - 1];
                        this.bo[j] = this.bo[i - 1] + 1;
                        if (l > 15) {
                            float f = 10.0F;
                            float f1 = 5.0F;
                            double d6 = MathHelper.a(this.random, this.locX - (double) f, this.locX + (double) f);
                            double d7 = MathHelper.a(this.random, this.locY - (double) f1, this.locY + (double) f1);
                            double d8 = MathHelper.a(this.random, this.locZ - (double) f, this.locZ + (double) f);

                            this.a(i + 1, d6, d7, d8, true);
                            this.bo[i - 1] = 0;
                        }
                    }
                    ...
                    for (int i1 = -1; i1 <= 1; ++i1) {
                        for (int j1 = -1; j1 <= 1; ++j1) {
                            for (int k1 = 0; k1 <= 3; ++k1) {
                                ...
                                if (block.getMaterial() != Material.AIR && a(block) && !CraftEventFactory.callEntityChangeBlockEvent(this, l1, i2, j2, Blocks.AIR, 0).isCancelled()) {
