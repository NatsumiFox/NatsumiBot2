...
public abstract class ExpirableListEntry<T> extends JsonListEntry<T> {
    ...
    private static <T> T checkExpiry(T t0, JsonObject jsonobject) {
        Date date = null;

        try {
            date = jsonobject.has("expires") ? ExpirableListEntry.a.parse(jsonobject.get("expires").getAsString()) : null;
        } catch (ParseException parseexception) {
            ;
        }

        return date != null && !date.after(new Date()) ? null : t0;
    }
}
