    ...
    public void tick(EntityLiving entityliving, int i) {
        if (this.id == MobEffectList.REGENERATION.id) {
            ...
            if (!entityliving.world.isClientSide) {
                ...
                int j = entityhuman.getFoodData().foodLevel;
                FoodLevelChangeEvent foodlevelchangeevent = CraftEventFactory.callFoodLevelChangeEvent(entityhuman, i + 1 + j);

                if (!foodlevelchangeevent.isCancelled()) {
                    entityhuman.getFoodData().eat(foodlevelchangeevent.getFoodLevel() - j, 1.0F);
                }
