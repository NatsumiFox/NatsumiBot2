...
public class PlayerChunkMap {
    ...
    public void flush() {
        ...
        if (i - this.h > 8000L) {
            ...
            while (iterator.hasNext()) {
                playerchunkmap_playerchunk = (PlayerChunkMap.PlayerChunk) iterator.next();
    ...
    public final boolean isChunkInUse(int i, int j) {
        PlayerChunkMap.PlayerChunk playerchunkmap_playerchunk = this.a(i, j, false);

        return playerchunkmap_playerchunk != null ? playerchunkmap_playerchunk.b.size() > 0 : false;
    }
    ...
    public void addPlayer(EntityPlayer entityplayer) {
        ...
        for (int k = i - this.g; k <= i + this.g; ++k) {
            for (int l = j - this.g; l <= j + this.g; ++l) {
                linkedlist.add(new ChunkCoordIntPair(k, l));
            }
        }

        Collections.sort(linkedlist, new PlayerChunkMap.ChunkCoordComparator(entityplayer));
        Iterator iterator = linkedlist.iterator();

        while (iterator.hasNext()) {
            ChunkCoordIntPair chunkcoordintpair = (ChunkCoordIntPair) iterator.next();

            this.a(chunkcoordintpair.x, chunkcoordintpair.z, true).a(entityplayer);
        }
                ...
                for (int l1 = i - i1; l1 <= i + i1; ++l1) {
                    for (int i2 = j - i1; i2 <= j + i1; ++i2) {
                        if (!this.a(l1, i2, k, l, i1)) {
                            linkedlist.add(new ChunkCoordIntPair(l1, i2));
                        }
    ...
    private static class ChunkCoordComparator implements Comparator<ChunkCoordIntPair> {
        ...
        public int compare(ChunkCoordIntPair chunkcoordintpair, ChunkCoordIntPair chunkcoordintpair1) {
            if (chunkcoordintpair.equals(chunkcoordintpair1)) {
                return 0;
            } else {
                int i = chunkcoordintpair.x - this.x;
                int j = chunkcoordintpair.z - this.z;
                int k = chunkcoordintpair1.x - this.x;
                int l = chunkcoordintpair1.z - this.z;
        ...
        public int compare(Object object, Object object1) {
            return this.compare((ChunkCoordIntPair) object, (ChunkCoordIntPair) object1);
        }
    }
