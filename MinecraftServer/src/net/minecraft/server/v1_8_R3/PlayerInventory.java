...
public class PlayerInventory implements IInventory {
    ...
    public int canHold(ItemStack itemstack) {
        ...
        for (int j = 0; j < this.items.length; ++j) {
            ...
            if (this.items[j] != null && this.items[j].getItem() == itemstack.getItem() && this.items[j].isStackable() && this.items[j].count < this.items[j].getMaxStackSize() && this.items[j].count < this.getMaxStackSize() && (!this.items[j].usesData() || this.items[j].getData() == itemstack.getData()) && ItemStack.equals(this.items[j], itemstack)) {
