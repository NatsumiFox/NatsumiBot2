...
public class WorldMap extends PersistentBase {
    ...
    public void a(NBTTagCompound nbttagcompound) {
        byte b0 = nbttagcompound.getByte("dimension");

        if (b0 >= 10) {
            long i = nbttagcompound.getLong("UUIDLeast");
            long j = nbttagcompound.getLong("UUIDMost");
        ...
        if (short0 == 128 && short1 == 128) {
            ...
            for (int i1 = 0; i1 < short1; ++i1) {
                ...
                if (j1 >= 0 || j1 < 128) {
                    for (int k1 = 0; k1 < short0; ++k1) {
                        int l1 = k1 + k;

                        if (l1 >= 0 || l1 < 128) {
                            this.colors[l1 + j1 * 128] = abyte[k1 + i1 * short0];
                        }
                    }
                }
            }
        ...
        if (this.map >= 10) {
            if (this.uniqueId == null) {
                ...
                while (iterator.hasNext()) {
                    org.bukkit.World org_bukkit_world = (org.bukkit.World) iterator.next();
                    CraftWorld craftworld = (CraftWorld) org_bukkit_world;
    ...
    public class WorldMapHumanTracker {
        ...
        public Packet a(ItemStack itemstack) {
            ...
            Iterator iterator = renderdata.cursors.iterator();

            while (iterator.hasNext()) {
                MapCursor mapcursor = (MapCursor) iterator.next();

                if (mapcursor.isVisible()) {
                    arraylist.add(new MapIcon(mapcursor.getRawType(), mapcursor.getX(), mapcursor.getY(), mapcursor.getDirection()));
                }
            }

            if (this.d) {
                this.d = false;
                return new PacketPlayOutMap(itemstack.getData(), WorldMap.this.scale, arraylist, renderdata.buffer, this.e, this.f, this.g + 1 - this.e, this.h + 1 - this.f);
            } else {
                return this.i++ % 5 == 0 ? new PacketPlayOutMap(itemstack.getData(), WorldMap.this.scale, arraylist, renderdata.buffer, 0, 0, 0, 0) : null;
            }
        }
